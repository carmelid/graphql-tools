...
describe('generating schema from shorthand', function () {
    ...
    describe('scalar types', function () {
        ...
        it('retains scalars after walking/recreating the schema', function () {
            var _a;
            var shorthand = "\n        scalar Test\n\n        type Foo {\n          testField: Test\n        }\n\n        type Query {\n          test: Test\n          testIn(input: Test): Test\n        }\n      ";
            var resolveFunctions = {
                Test: new graphql_1.GraphQLScalarType({
                    name: 'Test',
                ...
                Query: {
                    testIn: function (_, _a) {
                        var input = _a.input;
                        chai_1.expect(input).to.contain('scalar:');
                        return input;
                    },
                    test: function () {
                        return 42;
            ...
            var walkedSchema = visitSchema_1.visitSchema(makeExecutableSchema_1.makeExecutableSchema({
                typeDefs: shorthand,
                resolvers: resolveFunctions,
            }), (_a = {},
                _a[visitSchema_1.VisitSchemaKind.ENUM_TYPE] = function (type) {
                    return type;
                },
                _a));
            chai_1.expect(walkedSchema.getType('Test')).to.be.an.instanceof(graphql_1.GraphQLScalarType);
            chai_1.expect(walkedSchema.getType('Test'))
                .to.have.property('description')
                .that.equals('Test resolver');
            var testQuery = "\n        {\n          test\n          testIn(input: 1)\n        }";
            var resultPromise = graphql_1.graphql(walkedSchema, testQuery);
            return resultPromise.then(function (result) { return chai_1.expect(result.data).to.deep.equal({
                test: 'scalar:42',
                testIn: 'scalar:1'
            }); });
        });
    ...
    describe('enum support', function () {
        ...
        it('supports passing the value for a GraphQLEnumType in resolveFunctions', function () {
            ...
            var resultPromise = graphql_1.graphql(jsSchema, testQuery);
            return resultPromise.then(function (result) {
                chai_1.assert.equal(result.data['redColor'], 'RED');
                chai_1.assert.equal(result.data['blueColor'], 'BLUE');
                chai_1.assert.equal(result.data['numericEnum'], 'TEST');
                chai_1.assert.equal(result.errors, undefined);
            });
        });
        it('supports resolving the value for a GraphQLEnumType in input types', function () {
            ...
            var resolveFunctions = {
                ...
                Query: {
                    colorTest: function (root, args) {
                        return args.color;
                    },
                    numericTest: function (root, args) {
                        return args.num;
                    },
                },
            };
            ...
            var resultPromise = graphql_1.graphql(jsSchema, testQuery);
            return resultPromise.then(function (result) {
                chai_1.assert.equal(result.data['red'], resolveFunctions.Color.RED);
                chai_1.assert.equal(result.data['blue'], resolveFunctions.Color.BLUE);
                chai_1.assert.equal(result.data['num'], resolveFunctions.NumericEnum.TEST);
                chai_1.assert.equal(result.errors, undefined);
            });
        });
    });
...
describe('can specify lexical parser options', function () {
    ...
    xit("can specify 'experimentalFragmentVariables' option", function () {
